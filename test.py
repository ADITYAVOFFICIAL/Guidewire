...
# --- Configuration V7.3 ---
N_DATAPOINTS = 60000 # Target number of data points (Adjust as needed)
...
# <<< CHANGE START: Increased BASE_FAILURE_PROBABILITY >>>
# Original value was 0.00055
# Increase this value to generate more failure precursors relative to normal data.
# A value like 0.0025 (roughly 4.5x increase) might yield around 15-25% failure precursors.
# You might need to experiment with this value to achieve your desired balance.
BASE_FAILURE_PROBABILITY = 0.0050
# <<< CHANGE END >>>
...
# Note: This logic balances *among failure types*, not overall failure vs normal ratio.
...
def generate_base_value(metric: str, timestamp: datetime.datetime, current_n_nodes_total: int, target_total_pods: float, not_ready_node_ids: Set[int]) -> float:
    ...
    elif metric in ['Cluster_Network_Bytes_In', 'Cluster_Network_Bytes_Out', 'Cluster_Disk_IO_Rate', 'ErrorLogRate', 'WarningLogRate', 'OOMKillLogRate', 'NetworkErrorLogRate', 'EvictionLogRate', 'APIErrorLogRate']: # Includes specific log rates
    ...
    if metric in ['Num_Pods_Pending', 'Pod_Restart_Rate', 'Cluster_Network_Errors_Rate', 'App_Error_Rate_Avg', 'Evicted_Pod_Count_Rate', 'ImagePullBackOff_Count_Rate', 'CPU_Throttling_Rate', 'ErrorLogRate', 'WarningLogRate', 'PV_Attach_Error_Rate', 'OOMKillLogRate', 'NetworkErrorLogRate', 'EvictionLogRate', 'APIErrorLogRate']: # Includes specific log rates
...
class Application:
    ...
    def __init__(self, name: str):
        ...
        self.intended_replicas: int = 0 # Delayed replica updates logic
        self.replica_change_apply_step: int = -1 # Delayed replica updates logic
...
class Simulator:
    ...
    def run_step(self, i: int) -> Dict[str, Any]:
        ...
        self._apply_delayed_replica_changes() # Apply delayed replica changes BEFORE scheduling
    ...
    def _update_misc_cluster_metrics(self, step_metrics: Dict):
        ...
        if cs.current_state == STATE_PRECURSOR and cs.active_failure_type == 'Impending_API_Slowdown':
            ...
            if impact:
                delta, shape, delay_frac = impact[0], impact[1], impact[3] # Index 3 is exponent, should be delay (index 2)
                pv_error_increase = apply_trend(0, cs.progress, delta, shape) # Exponent missing here?
        ...
        # Ensure metrics exist before adding to them
        if 'CPU_Throttling_Rate' not in step_metrics: step_metrics['CPU_Throttling_Rate'] = 0
        if 'Evicted_Pod_Count_Rate' not in step_metrics: step_metrics['Evicted_Pod_Count_Rate'] = 0
            ...
            for impact_level in ['primary', 'secondary', 'tertiary']:
                 for metric, impact_data in failure_details.get(impact_level, {}).items():
                     if not any(keyword in metric for keyword in ['node_', 'app_']): # Apply cluster-wide metrics
                          ...
                          # Correctly unpack impact_data based on its expected length for the metric
                          base_value = step_metrics.get(metric)
                          if len(impact_data) == 2:
                              delta, shape = impact_data; exponent = 1.0; delay_frac = 0.0
                          elif len(impact_data) == 3: # Assuming (delta, shape, delay_frac) OR (delta, shape, exponent) -> Check type of last element if ambiguous, but delay is more common
                              delta, shape = impact_data[0], impact_data[1]
                              if isinstance(impact_data[2], (int, float)) and impact_data[2] <= 1.0: # Likely delay_frac
                                  delay_frac = impact_data[2]; exponent = 1.0
                              else: # Assume exponent
                                  exponent = impact_data[2]; delay_frac = 0.0
                          elif len(impact_data) == 4:
                              delta, shape, delay_frac, exponent = impact_data
                          else:
                              print(f"Warning: Unexpected impact_data format for {metric}: {impact_data}. Skipping impact.")
    ...
    def _apply_noise_and_finalize(self, step_metrics: Dict) -> Dict[str, Any]:
        ...
        for metric in self.metrics_to_record:
            ...
            elif 'Count' in metric or metric in ['Num_Pods_Running', 'Num_Pods_Pending', 'PV_Attached_Count', 'Num_Ready_Nodes', 'Num_NotReady_Nodes']: # Includes node counts
        ...
        for app_name, app_data in cs.apps.items():
            ...
            if app_data.replica_change_apply_step != -1 and cs.current_step >= app_data.replica_change_apply_step:
                ...
                if change < 0: # Handle scale down - remove pods from nodes
                    ...
                    # Note: This still sets current_replicas = intended below, reflecting the *controller's target*.
                    # If exact tracking of successfully removed pods is needed, logic might need adjustment.
                    pass # The setting below handles the target state

                # Update current replicas to the intended value
                # This reflects the controller's target state after the delay.
                app_data.current_replicas = intended
...
if __name__ == "__main__":
    ...
    print(f"Starting data generation V7.3 (Class Balance Adjusted) for {N_DATAPOINTS} steps...") # Version updated
    ...
    print(f"\nData generation V7.3 (Class Balance Adjusted) finished in {end_time_generation - start_time_generation:.2f} seconds.") # Version updated
    ...
    df = df[all_expected_cols]; output_filename = 'synthetic_kubernetes_metrics_realistic_v7_3_balanced.csv' # Version updated, new filename
    ...
    try:
        ...
        print("\nGenerating sample plots V7.3 (Class Balance Adjusted)...") # Version updated
        ...
        if not failures.empty:
            ...
            try: # Add try-except for safety if failures are sparse
                last_failure_group = unique_failure_blocks.get_group(failures['block'].iloc[-1])
                if not last_failure_group.empty:
                 ...
                 print(f"Note: Could not annotate last failure type on plot ({e_annot})")


        fig.autofmt_xdate()
        plt.tight_layout(rect=[0, 0.03, 1, 0.98]) # Adjust layout slightly for taller figure
        plot_filename = "synthetic_data_sample_plots_v7_3_balanced.png" # Version updated, new filename
        plt.savefig(plot_filename, dpi=150)
        print(f"\nSample plots V7.3 (Class Balance Adjusted) saved to {plot_filename}") # Version updated
